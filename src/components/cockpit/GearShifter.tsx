...
export const GearShifter = ({ currentGear, onGearChange }: GearShifterProps) => {
  return (
    <div className="flex flex-col items-center h-full py-2 px-3">
      <div className="racing-text text-xs text-muted-foreground mb-2">GEAR</div>
        ...
        {GEARS.map((gear) => {
          ...
          return (
            <button
              ...
              className={`
                w-12 h-8 sm:w-14 sm:h-9 rounded border text-sm font-bold racing-text
      ...
      {/* Telemetry Wave */}
      <div className="w-full mt-2 overflow-hidden h-8 border border-border rounded bg-card/50">
      ...
      <div className="text-[8px] text-muted-foreground racing-text mt-1">LIVE TELEMETRY</div>
    </div>
  );
};
