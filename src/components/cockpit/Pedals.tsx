...
export const Pedals = ({ onThrottleChange, onBrakeChange }: PedalsProps) => {
  ...
  return (
    <div className="flex h-full gap-2 p-2">
      ...
      >
        <div className={`racing-text text-lg sm:text-xl font-bold transition-all ${brakePressed ? 'text-white text-glow-red' : 'text-destructive-foreground/70'}`}>
          BRAKE
        </div>
        <div className="flex gap-1 mt-2">
          {[...Array(3)].map((_, i) => (
            <div 
              key={i}
              className={`w-2 h-1 rounded-full transition-colors ${brakePressed ? 'bg-white' : 'bg-destructive/50'}`}
            />
          ))}
        </div>
      </button>
      ...
      >
        <div className={`racing-text text-xl sm:text-2xl font-bold transition-all ${throttlePressed ? 'text-white text-glow-teal' : 'text-primary-foreground/70'}`}>
          THROTTLE
        </div>
        <div className="flex gap-1 mt-2">
          {[...Array(5)].map((_, i) => (
            <div 
              key={i}
              className={`w-3 h-1.5 rounded-full transition-all delay-${i * 50} ${
                throttlePressed 
                  ? 'bg-white' 
                  : 'bg-primary/50'
              }`}
        ...
        {/* Power Meter */}
        <div className="w-3/4 h-2 bg-card/50 rounded-full mt-3 overflow-hidden">
          <div 
